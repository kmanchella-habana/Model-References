...
#### Run training
```bash
$PYTHON demo_ssd.py -e <epoch> -b <batch_size> -d <precison> --model_dir <path/to/model_dir>
```
For example:
...
```bash
$PYTHON demo_ssd.py -e <epoch> -b <batch_size> -d <precison> --model_dir <path/to/model_dir> --hvd_workers 8
```
For example:
...
- Command using `demo_ssd.py`:
   ...
   mpirun --allow-run-as-root --bind-to core --map-by socket:PE=7 --np 8 -x HCL_CONFIG_PATH=hcl_config_8.json \
     $PYTHON /root/Model-References/TensorFlow/computer_vision/SSD_ResNet34/demo_ssd.py --use_horovod --hvd_workers=1 --epochs=50 --batch_size=128 --dtype=bf16 --model_dir=/tmp/ssd_8_hpus
   ```
...
Note that it is required to provide `--use_horovod` argument and provide HCL config file. For documentation on creating an HCL config JSON file, please refer to [HCL JSON Config File Format](https://docs.habana.ai/en/latest/API_Reference_Guides/HCL_API_Reference.html#hcl-json-config-file-format).

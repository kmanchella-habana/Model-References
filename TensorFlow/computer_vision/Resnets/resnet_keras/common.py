...
def define_keras_flags(
  ...
  flags.DEFINE_boolean(
      ...
      'HpuCollectiveReduce. Scoped Allocator-related operations bind together '
...
def define_habana_flags():
    ...
    DEFAULT_BF16_CONFIG_PATH = os.fspath(Path(os.path.realpath(__file__)).parents[3].joinpath("common/bf16_config/full.json"))
    flags.DEFINE_string(name='bf16_config_path', default=DEFAULT_BF16_CONFIG_PATH,
...
def initialize_preloading():
    if FLAGS.experimental_preloading:
        os.environ['HBN_TF_REGISTER_DATASETOPS'] = '1'
    else:
        os.environ.pop('HBN_TF_REGISTER_DATASETOPS', None)
